...
export class Find2chGasServiceRequestAdapter {
  adapt(value: GetThreadListRequest): string {
    const urlFactory = new Find2chSearchUrlFactory(
      value.searchWord,
      value.sortType,
      value.contentType,
      value.bbsType,
    ...
    return urlFactory.generate();
  }
}

class Find2chSearchUrlFactory {
  ...
  generate(): string {
    const url = `${this.baseUrl}?${this.urlSearchParams()}`;

    return url;
  }

  private urlSearchParams(): string {
    const params: { [key: string]: string; } = {};
    params['STR'] = this.encodeToEucJp(this.searchWord.value);
  ...
  private encodeToEucJp(value: string): string {
    return this.escapeEUCJP(value);
  }

  // TOOD: move function to the other file
  private escapeEUCJP(value: string): string {
    /**
    * Licensed under the MIT license.
    * https://github.com/Kuniwak/Escape-Codec-Library-for-Closure/blob/master/LICENSE
    */
    ...
    return value.replace(/[^*+.-9A-Z_a-z-]/g, (s) => {
